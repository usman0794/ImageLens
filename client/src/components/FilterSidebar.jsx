  ...
  return (
    <aside className="glass-panel p-5 lg:sticky lg:top-24">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
        ...
        <label className="space-y-2">
          <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">Source</span>
          <select className="input-field" value={filters.source} onChange={(event) => update('source', event.target.value)}>
            {sources.map((source) => (
              <option key={source} value={source}>{source === 'all' ? 'All' : source}</option>
            ))}
          </select>
        </label>
        ...
        <label className="space-y-2">
          ...
          <select className="input-field" value={filters.sortBy} onChange={(event) => update('sortBy', event.target.value)}>
